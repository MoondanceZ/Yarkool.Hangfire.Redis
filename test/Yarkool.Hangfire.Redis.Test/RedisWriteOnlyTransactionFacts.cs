...
namespace Yarkool.Hangfire.Redis.Test
{
    ...
    public class RedisWriteOnlyTransactionFacts
    {
        ...
        {
            Assert.Throws<ArgumentNullException>("storage", () => new RedisWriteOnlyTransaction(null!));
        }

        [Fact]
        public void Ctor_ThrowsAnException_WhenTransactionIsNull()
        {
            Assert.Throws<ArgumentNullException>("transaction", () => new RedisWriteOnlyTransaction(_storage));
        }
        ...
        public void ExpireJob_SetsExpirationDateForAllRelatedKeys()
        {
            UseConnection(redis =>
            {
                ...
                // Assert
                var jobEntryTtl = TimeSpan.FromSeconds(redis.Ttl("{hangfire}:job:my-job").GetValueOrDefault(0));
                var stateEntryTtl = TimeSpan.FromSeconds(redis.Ttl("{hangfire}:job:my-job:state").GetValueOrDefault(0));
                var historyEntryTtl = TimeSpan.FromSeconds(redis.Ttl("{hangfire}:job:my-job:state").GetValueOrDefault(0));
        ...
        public void PersistJob_RemovesExpirationDatesForAllRelatedKeys()
        {
            UseConnection(redis =>
            {
                ...
                // Assert
                Assert.Null(redis.Ttl("{hangfire}:job:my-job"));
                Assert.Null(redis.Ttl("{hangfire}:job:my-job:state"));
                Assert.Null(redis.Ttl("{hangfire}:job:my-job:history"));
            });
        }
        ...
        public void AddToQueue_AddsSpecifiedJobToTheQueue()
        {
            UseConnection(redis =>
            {
                ...
                Assert.Equal("my-job", redis.LIndex("{hangfire}:queue:critical", 0));
            });
        }
        ...
        public void IncrementCounter_IncrementValueEntry()
        {
            UseConnection(redis =>
            {
                ...
                Assert.Equal("4", redis.Get("{hangfire}:entry"));
                Assert.Null(redis.Ttl("{hangfire}:entry"));
            });
        }
        ...
        public void IncrementCounter_WithExpiry_IncrementsValueAndSetsExpirationDate()
        {
            UseConnection(redis =>
            {
                ...
                var entryTtl = TimeSpan.FromSeconds(redis.Ttl("{hangfire}:entry").GetValueOrDefault(0));
                Assert.Equal("4", redis.Get("{hangfire}:entry"));
        ...
        public void DecrementCounter_DecrementsTheValueEntry()
        {
            UseConnection(redis =>
            {
                ...
                Assert.Equal("2", redis.Get("{hangfire}:entry"));
                Assert.Null(redis.Ttl("{hangfire}:entry"));
            });
        }
        ...
        public void RemoveFromSet_RemoveSpecifiedItemFromSortedSet()
        {
            UseConnection(redis =>
            {
                redis.ZAdd("{hangfire}:my-set", "my-value", 0);
        ...
        public void InsertToList_PrependsListWithSpecifiedValue()
        {
            UseConnection(redis =>
            {
                ...
                Assert.Equal("new-value", redis.LIndex("{hangfire}:list", 0));
            });
        }
        ...
        public void RemoveFromList_RemovesAllGivenValuesFromList()
        {
            UseConnection(redis =>
            {
                ...
                Assert.Equal("another-value", redis.LIndex("{hangfire}:list", 0));
            });
        }
        ...
        public void TrimList_TrimsListToASpecifiedRange()
        {
            UseConnection(redis =>
            {
                ...
                Assert.Equal("2", redis.LIndex("{hangfire}:list", 0));
                Assert.Equal("3", redis.LIndex("{hangfire}:list", 1));
            });
        }
        ...
        {
            UseConnection(redis => { Assert.Throws<ArgumentNullException>("key", () => Commit(redis, x => x.SetRangeInHash(null!, new Dictionary<string, string>()))); });
        }
        ...
        {
            UseConnection(redis => { Assert.Throws<ArgumentNullException>("keyValuePairs", () => Commit(redis, x => x.SetRangeInHash("some-hash", null!))); });
        }
        ...
        {
            UseConnection(redis => { Assert.Throws<ArgumentNullException>(() => Commit(redis, x => x.RemoveHash(null!))); });
        }
        ...
        private void Commit(IRedisClient redis, Action<RedisWriteOnlyTransaction> action)
        {
