...
namespace Yarkool.Hangfire.Redis.Test
{
    ...
    public class FetchedJobsWatcherFacts
    {
        ...
        {
            Assert.Throws<ArgumentNullException>("storage", () => new FetchedJobsWatcher(null!, InvisibilityTimeout));
        }

        [Fact]
        public void Ctor_ThrowsAnException_WhenInvisibilityTimeoutIsZero()
        {
            Assert.Throws<ArgumentOutOfRangeException>("invisibilityTimeout", () => new FetchedJobsWatcher(_storage, TimeSpan.Zero));
        }

        [Fact]
        public void Ctor_ThrowsAnException_WhenInvisibilityTimeoutIsNegative()
        {
            Assert.Throws<ArgumentOutOfRangeException>("invisibilityTimeout", () => new FetchedJobsWatcher(_storage, TimeSpan.FromSeconds(-1)));
        }
        ...
        public void Execute_EnqueuesTimedOutJobs_AndDeletesThemFromFetchedList()
        {
            ...
            redis.HSet("{hangfire}:job:my-job", "Fetched", JobHelper.SerializeDateTime(DateTime.UtcNow.AddDays(-1)));
            ...
            var listEntry = redis.RPop("{hangfire}:queue:my-queue");
        ...
        public void Execute_MarksDequeuedJobAsChecked_IfItHasNoFetchedFlagSet()
        {
            ...
            Assert.NotNull(JobHelper.DeserializeNullableDateTime(redis.HGet("{hangfire}:job:my-job", "Checked")));
        }
        ...
        public void Execute_EnqueuesCheckedAndTimedOutJob_IfNoFetchedFlagSet()
        {
            ...
            redis.SAdd("{hangfire}:queues", "my-queue");
            redis.RPush("{hangfire}:queue:my-queue:dequeued", "my-job");
            redis.HSet("{hangfire}:job:my-job", "Checked", JobHelper.SerializeDateTime(DateTime.UtcNow.AddDays(-1)));
