...
namespace Yarkool.Hangfire.Redis.Test
{
    ...
    public class RedisConnectionFacts
    {
        ...
        {
            UseConnection(connection => Assert.Throws<ArgumentNullException>("jobId", () => connection.GetStateData(null!)));
        }
        ...
        public void GetStateData_ReturnsNullReason_IfThereIsNoSuchKey()
        {
            UseConnections((redis, connection) =>
            {
                redis.HSet(
                    "{hangfire}:job:my-job:state",
                    new Dictionary<string, string> { { "State", "Name" } });
        ...
        {
            UseConnection(connection =>
                Assert.Throws<ArgumentNullException>("key", () => connection.GetAllItemsFromSet(null!)));
        }
        ...
        public void GetAllItemsFromSet_ReturnsAllItems()
        {
            ...
            {
                // Arrange
                redis.ZAdd("{hangfire}:some-set", "1", 0);
                redis.ZAdd("{hangfire}:some-set", "2", 0);
        ...
        {
            UseConnection(connection => { Assert.Throws<ArgumentNullException>("key", () => connection.SetRangeInHash(null!, new Dictionary<string, string>())); });
        }
        ...
        {
            UseConnection(connection => { Assert.Throws<ArgumentNullException>("keyValuePairs", () => connection.SetRangeInHash("some-hash", null!)); });
        }
        ...
        {
            UseConnection(connection => Assert.Throws<ArgumentNullException>(() => connection.GetAllEntriesFromHash(null!)));
        }
        ...
        private void UseConnections(Action<IRedisClient, RedisConnection> action)
        {
            ...
            using (var connection = new RedisConnection(_storage, subscription, new RedisStorageOptions().FetchTimeout))
            {
